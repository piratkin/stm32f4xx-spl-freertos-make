...
PRIVILEGED_DATA TCB_t * volatile pxCurrentTCB = NULL;
...
#if ( configUSE_TRACE_FACILITY == 1 )
    UBaseType_t getCurTaaskNum(void)
    {
        return pxCurrentTCB->uxTCBNumber;
    }

    char* getCurTaaskName(void)
    {
    ...
    void vSwitchedTask(void)
    {
        tasks = (UINT) uxTaskGetNumberOfTasks();
        cur_task_num = (UINT) getCurTaaskNum();
        
        ps_arr[cur_task_num - 1]++;
        
        //set name task
        if (nm_arr[cur_task_num - 1] == 0)
        {
            nm_arr[cur_task_num - 1] = getCurTaaskName();
        }
        
        if (++counter > pdMS_TO_TICKS(1000))
        {
            for (UINT i = 0; i < tasks; i++)
            {
                ...
                if (max_inx > tasks)
                {
                    //rt_printf_usb("\n%2d. - %5d - %s", max_val, i, nm_arr[max_inx][0]);
                    //ps_arr[max_inx] = 0;
                }
            }
            counter = 0;
